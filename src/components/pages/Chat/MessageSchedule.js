...
function MessageSchedule({ message, align }) {
  ...
  const handleApprove = async (message) => {
    ...
    if (update_interview_status.fulfilled.match(actionResult)) {
      firebaseService.updateStatusInterview(
        chatId,
        message.message.interview_id,
        APPROVAL
      );
      ...
      <div className={`${containerClass}`}>
        <Box ml={align === "left" ? 2 : "auto"} className="message_content">
          <div className="message_schedule_container">
            ...
            <Typography variant="body1" className="schedule-content">
              <b>Time: </b>
              {message.message.hour_start.toString().padStart(2, "0")}:
              {message.message.minute_start.toString().padStart(2, "0")}
              <span> - </span>
              {message.message.hour_end.toString().padStart(2, "0")}:
              {message.message.minute_end.toString().padStart(2, "0")}{" "}
            </Typography>
            <Typography variant="body1" className="schedule-content">
              <b>Date: </b>
