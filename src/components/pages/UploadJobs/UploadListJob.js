...
import JobItem from "./UploadJobItem";
        ...
        <List key="job_upload-item">
          <div onClick={handleToggleActiveJobs} className="active-chevrondown">
            ...
            {showActiveJobs ? (
              ...
              <FaAngleRight className="custom-chevron" />
            )}
          </div>
          {showActiveJobs &&
            activeJobs.map((job) => <JobItem key={job.id} job={job} />)}
          <div
            onClick={handleToggleDeactiveJobs}
            className="active-chevrondown"
          ...
          {showDeactiveJobs &&
            inactiveJobs.map((job) => <JobItem key={job.id} job={job} />)}
        </List>
      </div>
    </div>
  );
