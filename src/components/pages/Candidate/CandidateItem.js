...
const CandidateItem = ({ candidate, onCandidateClick }) => {
  ...
  return loading ? (
    ...
    <div
      className="candidate_item-container"
      onClick={(e) => {
        e.preventDefault();
    ...
    >
      <ListItem key={candidate.id}>
        ...
        <div className="candidate_item-content">
          ...
          <Typography variant="body1" className="email-text">
            <MailIcon />
            {`: ${candidate.email}`}
          </Typography>
          ...
          <Typography variant="body1" className="skill-text">
            <BiotechIcon />
            {`: ${candidate.skills}`}
          </Typography>
        </div>
      ...
      >
        <Modal
          open={modalOpen}
          onClose={handleCloseModalJob}
          className="modal-list-job"
        >
          <div className="job_upload_item-content  job_upload_item_container ">
            <button className="close-button" onClick={handleCloseModalJob}>
              X
            </button>
            <Typography variant="h6">Select a job</Typography>
            {activeJobs.map((job) => (
              <div
                className={`job_upload_item-content ${
                  selectedItem?.id === job.id ? "job_upload_item_selected" : ""
                }`}
                key={job.id}
                onClick={(e) => {
                  e.stopPropagation();
              ...
              >
                <Typography variant="subtitle1">{job.job_name}</Typography>
                <Typography variant="body1" className="company-loc-text">
                  <LocationOnIcon />
                  {job.location}
                </Typography>
